...
class StreamQuery(Sequence):
    ...
    def get_by_resolution(self, resolution: str) -> Optional[Stream]:
        ...
        """
        #print(self.filter(progressive=True))
        return self.filter(
            progressive=True, subtype="mp4", resolution=resolution
        ).first()
		
    def get_by_resolution_limitless(self, resolution: str) -> Optional[Stream]:
        return self.filter(
            progressive=True, resolution=resolution
        ).first()
    ...
    def get_lowest_resolution_limitless(self) -> Optional[Stream]:
        return (
